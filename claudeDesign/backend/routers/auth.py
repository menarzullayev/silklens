...
async def google_sign_in(
    payload: GoogleTokenRequest,  # noqa: ARG001  (used once credentials land)
    db: SessionDep,  # noqa: ARG001  (used once credentials land)
) -> LoginResponse:
    """Sign in or register via Google ID token.

    Verifies the token with Google, creates/fetches user, returns JWT pair.
    Currently stubbed — returns mock response until Google credentials configured.
    """
    # TODO(oauth): verify payload.id_token with google-auth library
    # For now return a structured error so Flutter knows the endpoint exists
    raise HTTPException(
        status_code=501,
        detail={
            "code": "GOOGLE_OAUTH_NOT_CONFIGURED",
            "message": (
                "Google OAuth credentials not yet configured. "
