...
void main() {
  group("HybridLogicalClock", () {
    ...
    test("receive merges remote correctly", () {
      final local = HybridLogicalClock(physical: 1000, counter: 1, nodeId: "aaaa");
      final remote = HybridLogicalClock(physical: 2000, counter: 3, nodeId: "bbbb");
      final merged = local.receive(remote);
      expect(merged.physical >= 2000, isTrue);
    });

    test("pack/parse round-trip preserves fields", () {
      final hlc = HybridLogicalClock(physical: 123, counter: 4, nodeId: "deadbeef");
